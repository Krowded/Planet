...
void display(void)
{
	...
	{
		total = Mult(camMatrix, TerrainModelToWorld[i]);
	...
	vec3 offvec = GetCurrentPosition(TerrainModelToWorld[0]); //Has to be the model matrix for the one you're walking on
	...
	ModelToWorld = AdjustModelToHeightMap(ModelToWorld, 
		 								  GetTerrainHeight(currentPosition, terrainModel, terrainTexture));
